...
function complexTests(){
  ...
  // CALL THE FUNCTION GENERATING THE MANDLEBROT SET
	generateMandlebrot(xMin, yMin, maxIterations, LIMIT);
...
function mandlebrot(x, y, iterations, limit){
  ...
  // FOR LOOP ITERATES UNTIL THE MAX ITERATIONS
	for (let i = 0; i < iterations; i++){

		// CALCULATE Z^2 (z multiplied by z)
		let zSqr = z.multiply(z);
		// THEN SUBTRACT THE COMPLEX CONSTANT (x0, y0)
		let cResult = zSqr.sub(c);
    // MODULO THE RESULT (ABSOLUTE VALUE)
		let cMod = cResult.modulo();
		// CHECK IF THIS MODULE IS ABOVE THE LIMIT
		if (cMod >= limit){
			break;
		}else{
			// IF NOT, FEED THIS BACK INTO THE FORMULA
			z = cResult;
